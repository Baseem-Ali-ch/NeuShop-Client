...
import { ChevronDown, ChevronUp, Edit, Copy, Trash2, Eye, MoreHorizontal, ArrowUpDown, Check, X } from "lucide-react"
...
export default function ProductTable({ searchTerm, filters, onEditProduct }: ProductTableProps) {
  ...
  const handleToggleStatus = (productId: number, isActive: boolean) => {
    setProducts(
      products.map((product) =>
        product.id === productId ? { ...product, status: isActive ? "active" : "draft" } : product,
      ),
    )
  }
  ...
  const handleNameSave = () => {
    if (editingName) {
      setProducts(
        products.map((product) => (product.id === editingName.id ? { ...product, name: editingName.name } : product)),
      )
      setEditingName(null)
    }
  }
          ...
          <Table>
            <TableHeader className="bg-background/50">
              <TableRow>
                <TableHead className="w-12">
                  <Checkbox
                    checked={
                      ...
                      getCurrentPageItems().every((product) => selectedProducts.includes(product.id))
                    }
                ...
                <TableHead>
                  <div className="flex items-center cursor-pointer" onClick={() => handleSort("categoryId")}>
                    Category
                    <ArrowUpDown className="ml-1 h-4 w-4" />
                  </div>
                </TableHead>
                ...
                <TableHead>
                  <div className="flex items-center cursor-pointer" onClick={() => handleSort("unitsSold")}>
                    Sales
                    <ArrowUpDown className="ml-1 h-4 w-4" />
                  </div>
                </TableHead>
            ...
            <TableBody>
              {getCurrentPageItems().length === 0 ? (
                ...
                getCurrentPageItems().map((product) => (
                  <TableRow key={product.id} className="hover:bg-background/40 transition-colors group">
                    <TableCell>
                      <Checkbox
                    ...
                    <TableCell>
                      ...
                      <div className="text-xs text-muted-foreground line-clamp-1 mt-1">{product.description}</div>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-col space-y-1">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-muted-foreground">/ {product.stock + 100}</span>
                        </div>
                        <Progress
                          value={(product.stock / (product.stock + 100)) * 100}
                          className="h-2"
                          indicatorClassName={getInventoryColor(product.stock, product.lowStockThreshold)}
                        />
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <div>{product.unitsSold} units</div>
                        <div className="text-muted-foreground">${(product.unitsSold * product.price).toFixed(2)}</div>
                      </div>
                    </TableCell>
