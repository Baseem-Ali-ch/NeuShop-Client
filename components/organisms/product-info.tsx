...
export default function ProductInfo({
  ...
  const sku = product.sku || `SKU-${product._id.$oid.padStart(6, "0")}`;
  ...
  // Color options: Include "Default" and unique variant colors
  const variantColors = product.variants?.find((v) => v.type.toLowerCase() === "color")?.options.map((opt) => opt.value) || [];
  const colorOptions = ["Default", ...variantColors.filter((color) => color !== "Default")];
  // Size options: Only if size variants exist
  const sizeOptions = product.variants?.find((v) => v.type.toLowerCase() === "size")?.options.map((opt) => opt.value) || [];
    ...
    >
          ...
          <div className="flex flex-wrap gap-4">
            {colorOptions.map((color) => (
              <div key={color} className="flex flex-col items-center gap-1">
                <button
                  className={cn(
                    "group relative w-12 h-12 rounded-full focus:outline-none",
                    selectedColor === color
                      ? "ring-4 ring-gray-900 dark:ring-gray-100"
                      : "ring-1 ring-transparent hover:ring-gray-500 dark:hover:ring-gray-400"
                  )}
                  ...
                  {selectedColor === color && (
                    <span className="absolute inset-0 flex items-center justify-center">
                      <Check
                        className={cn(
                          "h-5 w-5",
                          colorMap[color] === "#FFFFFF" || color === "#F5F5DC"
                            ? "text-gray-900"
                            : "text-white"
                        )}
                      />
            ...
            {sizeOptions.map((size) => (
              <button
                ...
                className={cn(
                  ...
                  "backdrop-blur-sm",
                  selectedSize === size
                    ? "bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900 ring-4 ring-gray-900 dark:ring-gray-100"
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-8">
        ...
        <Button
          ...
          onClick={onAddToCart}
          disabled={isAddingToCart || product.stock <= 0 || (colorOptions.length > 0 && !selectedColor)}
        >
          {isAddingToCart ? (
            <>
